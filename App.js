    ...
    const inventors = [
      ...
      const inventorsInFifteen = inventors.filter(function (inventor) {
        if (inventor.year > 1499 && inventor.year < 1600) {
          return inventor.year; // else return false
        }
      });
  
      console.table(inventorsInFifteen);
  
  
      // Array.prototype.map()
      // 2. Give us an array of the inventors first and last names
  
      const fullNames = inventors.map(fullName => `${fullName.first} ${fullName.last}`);
      ...
      const yearsLived = inventors.reduce(function(total, age){
